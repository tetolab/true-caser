...
def training_data_generator(text_generator, sentence_length, batch_size, shift=True):
        ...
        for sentence in batch(text, sentence_length):
            try:
                if shift:
                    ...
                    sentence = pad(sentence, sentence_length, ' ')
...
def conll_shifted_sentence_generator(padding_symbol=0, max_shift_len=32):
    for sentence in conll_sentence_generator():
        for shifted_sentence in generate_shifted_data(sentence, max_len_of_sentence=max_shift_len, padding_symbol=padding_symbol):
